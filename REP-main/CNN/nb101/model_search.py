    ...
    def forward(self, s0, weights, output_weights, input_weights):
        ...
        for choice_block_idx in range(self._steps):
            ...
            if input_weights is not None:
                ...
                if (choice_block_idx == 0) or (choice_block_idx == 1):
                    input_weight = None
                else:
...
class Network(nn.Module):
    ...
    def __init__(self, C, num_classes, layers, criterion, output_weights, search_space, steps):
    ...
    def genotype(self):
        #mp conv3 conv1
        op_list = self._arch_parameters[0].data.cpu().numpy()
        for node in op_list:
            maxop = 0
            for op in range(len(node)):
                if node[op] > node[maxop]:
                    maxop = op
            for op in range(len(node)):
                if op == maxop:
                    node[op] = 1
        ...
        edges = []
        for input_weights in self._arch_parameters[2: 6]:
            weights = input_weights.data.cpu().numpy()
            maxedge = 0
            for edge in range(len(weights[0])):
                if input_weights[0][edge] > input_weights[0][maxedge]:
                    maxedge = edge
            for edge in range(len(weights[0])):
                if edge == maxedge:
                    weights[0][edge] = 1
